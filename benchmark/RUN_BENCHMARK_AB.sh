...
BASE=("${COMPAND_ARGS[@]}" "${INPUT_ARGS[@]}"
      ...
      --cpu-wavelet-fused on --cpu-split-fused on --cpu-split-neon "${AB_SPLIT_NEON:-on}"
      --cpu-gpr-raw-copy "${AB_RAWCOPY:-off}"
      ...
      --win-lowpass-bulk "${AB_LPBULK:-off}" --vle-prequant-skip off
      --cpu-gpr-helper off
      --compand-inframe-bits "${COMPAND_BITS:-12}"
      --cpu-winner on
      --cpu-input-prefetch "${AB_PREFETCH:-on}"
